...
class ModelCommunicationReportEmail extends Model 
{  

	public function getTotalReportEmail($type=NULL) {
		$search = '';
		if($type !=""){
	...
	public function getReportEmail($data = array(),$type=NULL) {
	...
	public function getTemplateName($template_id) { 	
 		$query = $this->db->query("SELECT subject FROM " . DB_PREFIX . "communication_template_email WHERE communication_template_email_id = $template_id ");	
		return $query->row['subject'];
	}
	public function getResultSend($communication_campaign_id) {	 		
		$query = $this->db->query("SELECT COUNT(status) as total FROM " . DB_PREFIX . "communication_email_send_details WHERE communication_campaign_id = $communication_campaign_id AND status = 1 ");	
		return $query->row['total'];		
	}
	public function getResultNotSend($communication_campaign_id) {	 		
		$query = $this->db->query("SELECT COUNT(status) as total FROM " . DB_PREFIX . "communication_email_send_details WHERE communication_campaign_id = $communication_campaign_id AND status = 0 ");	
		return $query->row['total'];		
	}
}
?>