    ...
    <div class="box" style="width:73%;float:left">
      ...
      <div class="content">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
          <table width="100%">
            <tr>
              <td width="50%" valign="top"><table class="form">
            	  <tr style="display:none">
              <td><span class="required">*</span><?php echo $entry_notification_type; ?></td>
              <td><select id="notification_type" name="notification_type">                 
                  <option value="customer" <?php echo $notification_type=='customer'?' selected="selected"':''; ?> >Customer</option>
                </select>
                <?php if ($error_notification_type) { ?>
                <span class="error"><?php echo $error_notification_type; ?></span>
                <?php } ?></td>
            </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td><textarea id="message" name="message" cols="200" rows="30"><?php echo isset($message) ? $message : ''; ?></textarea>
                      <table width="100%">
                        <tr>
                          <td></td>
                          <td align="right"><a href="javascript://" id="import_notification_templates" class="button">Import</a></td>
                        </tr>
                      </table>
...
function getFileContent(filename){
	$.ajax({
		...
		url: 'index.php?route=communication/template_notification/getFileData&token=<?php echo $this->session->data["token"]; ?>',			
...
$(document).ready(function(e) {
		...
		url: 'index.php?route=communication/template_notification/getTagList&token=<?php echo $this->session->data["token"]; ?>',			
		success: function(data,status){ 			
...
// Get List of File in Notification Templates
    $("#import_notification_templates").click(function(){		
		$.ajax({
			type: "POST",
			url: 'index.php?route=communication/template_notification/getAllTemplates&token=<?php echo $this->session->data["token"]; ?>',			
			success: function(data,status){ 			
				 // Dispaly File List 
